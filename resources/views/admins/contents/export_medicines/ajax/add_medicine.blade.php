@if(isset($medicine))
    <tr class="row_sell" data-id_selected="{{$medicine->id}}">
        ...
        <td>
            <select class="form-control units_select" name="unit_id[]">
                @if(count($units = $medicine->units)>0)
                    @foreach($units as $k=>$unit)
                        <option
                            value="{{$unit->id}}" {{$k==0?"selected":''}} data-unit_price="{{$unit->price}}">{!! $unit->name !!} </option>
                    @endforeach
                @endif
            </select>
        </td>
        <td class="" data-total_price="0">
            <input type="text" class="price_row amount_medicine border-0" name="price[]"
